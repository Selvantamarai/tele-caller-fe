...
export class Validators {
  static get(validation: Validation): (name: string, value: any) => string[] {
    const result = (name: string, value: any) =>
      Validators.validate(name, value, validation);
    return result;
  }
  ...
  static validate(name: string, value: any, validation: Validation): string[] {
    ...
    if (validation.minLength && length < validation.minLength) {
      temp.push(
        `${name} has to be at least ${validation.minLength} characters long. ActualLength: ${length}`
      );
    }
    if (validation.maxLength && length > validation.maxLength) {
      temp.push(
        `${name} can't be longer then ${validation.maxLength} characters. ActualLength: ${length}`
      );
    }
    if (validation.pattern && value) {
      const patternResult = Validators.patternValidate(
        name,
        value,
        validation.pattern
      );
  ...
  static patternValidate(
    ...
    return regex.test(value)
      ? null
      : `${name} must match this pattern: ${regexStr}.`;
  }
}
